...
This node is the SINGLE owner of USB access to the ReSpeaker hardware registers.
Nothing else in the stack may open the device: the XVF-3000 does not arbitrate
concurrent control transfers, so a second claimant makes reads intermittently
fail. It therefore replaces the old respeaker_doa_node (DOA) and serves
vad_node's hardware VAD over the 'hardware_vad' topic instead of a direct
USB read.
...
class XmosHardwareNode(Node):
    def __init__(self):
        ...
        # DOA goes out on 'doa_angle', not gemini's 'hardware_doa': this branch
        # already has consumers on that topic (openai_realtime_node's attention
        # focus and pipecat_audio_node), and this node replaces the standalone
        # respeaker_doa_node that used to feed them.
        self.doa_pub = self.create_publisher(Int32, 'doa_angle', 10)
...
def main(args=None):
    ...
    try:
        ...
        # Raised on SIGTERM (launch teardown, systemd stop). Without this the
        # node exits with a traceback on every normal shutdown.
        pass
    finally:
        node.destroy_node()
        try:
