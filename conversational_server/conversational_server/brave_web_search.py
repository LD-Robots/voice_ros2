...
def normalize_brave_country(country: str) -> str:
    """Return a Brave-compatible country code."""
    normalized = str(country or '').strip().upper()
    if not normalized:
        return DEFAULT_WEB_SEARCH_COUNTRY
    if normalized in _SUPPORTED_COUNTRIES:
        return normalized
    # Some countries (for example RO) are not accepted by the LLM Context enum.
    # Use ALL so search stays functional instead of returning HTTP 422.
    return 'ALL'
...
def call_brave_web_search(
    ...
    *,
    ...
    payload = {
        'q': query,
        'country': normalize_brave_country(country),
        'search_lang': (
